...
$employee_columns = [];
$column_result = mysqli_query($conn, "SHOW COLUMNS FROM employees");
if ($column_result) {
    while ($column = mysqli_fetch_assoc($column_result)) {
        $employee_columns[$column['Field']] = true;
    }
}

$exclude_resigned = [];
if (isset($employee_columns['employee_status'])) {
    $exclude_resigned[] = "(employee_status IS NULL OR employee_status = '' OR employee_status NOT IN ('Resign', 'Resigned'))";
}
if (isset($employee_columns['status'])) {
    $exclude_resigned[] = "(status IS NULL OR status = '' OR status NOT IN ('Resign', 'Resigned'))";
}
if (isset($employee_columns['resign_date'])) {
    $exclude_resigned[] = "(resign_date IS NULL OR resign_date = '' OR resign_date = '0000-00-00' OR resign_date > '$today')";
}
$exclude_resigned_sql = $exclude_resigned ? " AND " . implode(" AND ", $exclude_resigned) : "";
...
$result = mysqli_query($conn,"
    ...
    AND insurance_expiry_date BETWEEN '$today' AND '$three_months'
    $exclude_resigned_sql
...
<title>Insurance Expiring Soon</title>
<style>
...
<div class="topbar">
    ...
    <div class="topbar-right">
        ...
        <span class="count-pill">&#128737; <?php echo $total_count; ?> Insurance<?php echo $total_count > 1 ? 's' : ''; ?> Expiring</span>
        <?php endif; ?>
...
<div class="page-body">
    ...
    <div class="page-heading">
        <div class="icon">&#128196;</div>
        <h1>Insurance Expiring Within 3 Months</h1>
    </div>
    ...
    if ($total_count > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            $remaining = floor((strtotime($row['insurance_expiry_date']) - strtotime($today)) / 86400);
    ...
    <tr class="<?php echo $urg; ?>">
        ...
        <td><?php echo h(trim((string)($row['insurance_number'] ?? '')) !== '' ? $row['insurance_number'] : 'No Insurance Number'); ?></td>
        <td class="date-cell"><?php echo h(display_date_dmy($row['insurance_expiry_date'])); ?></td>
    ...
    <tr>
        <td colspan="8">
            <div class="empty-state">
                <div class="icon">&#9989;</div>
                <h3>All Clear!</h3>
                <p>No employee insurance expiring within the next 3 months.</p>
            </div>
        </td>
    </tr>
    <?php } ?>
